...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        bool LOST_BUFFER_JPEGHeader = buffer[0] == 0xff && buffer[1] == 0xd8 && buffer[2] == 0xff && (buffer[3] & 0xf0) == 0xe0;

        // if we found a yet another JPEG, we must close the previous file
        if (LOST_BUFFER_JPEGHeader && outptr != NULL)
        {
            fclose(outptr);
            imageCount++;
        }
