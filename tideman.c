...
//You already know about plurality elections, which follow a very simple algorithm for determining the winner of an election: every voter gets one vote,
...
int comparator(const void *a, const void *b)
{
    ...
    return (PREFER[orderB->winner][orderB->loser] - PREFER[orderA->winner][orderA->loser]);
}
