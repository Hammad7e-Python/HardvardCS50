...
with open(argv[2]) as DNA_Mainfile:
    dnareader = reader(DNA_Mainfile)
...
with open(argv[1]) as ppl_files:
    people = reader(ppl_files)
    for row in people:
        dnaSequences = row
        dnaSequences.pop(0)
        break
...
with open(argv[1], newline='') as ppl_files:
    people = DictReader(ppl_files)
